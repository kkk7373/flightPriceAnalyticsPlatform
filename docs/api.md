...
現在の`GET /api/init`はリクエストパラメータを受け取らず、Backend内の固定SQLを実行します。同じ日本時間の基準日に対する結果はアプリ内でキャッシュし、有効なキャッシュがあればAthenaクエリを実行しません。Athena Queryの失敗やタイムアウトは共通ハンドラでエラーレスポンスに変換します。Frontendから任意SQLを送信する機能は提供しません。
...
Athenaクエリの失敗・キャンセル、またはAWS SDKの例外はHTTP `502`、クエリ完了待ちのタイムアウトはHTTP `504`を返します。本文は`application/problem+json`で、例外の内部メッセージは含めません。
...
```json
{
  ...
  "title": "Bad Gateway",
  "status": 502,
  "detail": "集計データを取得できませんでした",
  "instance": "/api/init",
  "code": "ATHENA_QUERY_FAILED"
}
```

タイムアウト時は`status: 504`、`code: "ATHENA_TIMEOUT"`、`detail: "データ取得がタイムアウトしました"`となり、実行中のAthenaクエリの停止を試みます。停止に失敗した場合もHTTP `504`を返し、失敗内容はログに記録します。このタイムアウトは各クエリの完了待ちに適用され、4クエリを含むAPI全体の制限時間ではありません。

Athena結果を変換できない場合や当日集計行が欠けている場合はHTTP `502`、SQLリソースの読み込み失敗はHTTP `500`（`SQL_RESOURCE_ERROR`）、処理中断はHTTP `503`（`REQUEST_INTERRUPTED`）、その他の予期しない実行時例外はHTTP `500`（`INTERNAL_ERROR`）のProblem Detailを返します。いずれも例外の内部メッセージは本文に含めません。Athenaクエリが正常に完了しても対象データが0件の場合は、エラーではなくHTTP `200`を返します。
